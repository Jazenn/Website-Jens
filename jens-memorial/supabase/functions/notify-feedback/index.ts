...
Deno.serve(async (request) => {
  ...
  try {
    const { email, name, type, message } = await request.json()

    if (!email || !type || !message) {
      return new Response(JSON.stringify({ error: 'Email, type, and message are required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const typeLabel = {
      compliment: 'Compliment ❤️',
      problem: 'Probleem / Vraag ❓',
      error: 'Foutmelding / Bug 🐛',
      other: 'Overig 📝',
    }[type as string] ?? type
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `Nieuw bericht (${typeLabel}) op Website Jens`,
        html: `
        ...
        `,
        text: `Nieuw bericht (${typeLabel}) op Website Jens\n\nNaam: ${displayName}\nE-mail: ${email}\n\nBericht:\n${message}${adminUrl ? `\n\nAdminpaneel: ${adminUrl}` : ''}`,
      }),
    })
